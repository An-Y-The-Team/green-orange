...
test("returns the mapped label and variant for a known key", () => {
  expect(labelOf(projectStage, ProjectStage.CLOSED)).toEqual({
    label: "Đã đóng",
    variant: "success",
  });
});

// The bug this guards: `projectStage[project.stage].label` on a stage the enum
...
test("falls back to the raw key instead of throwing on an unmapped key", () => {
  ...
  expect(labelOf(projectStage, unknown)).toEqual({
    label: "khao_sat",
    variant: "secondary",
  });
