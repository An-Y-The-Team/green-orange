...
export async function headlessLogin(
  ...
  try {
    ...
    if (!slug) return { ok: false, reason: LoginFailureReason.ERROR };
    ...
    for (let i = 0; i < 6; i++) {
      ...
      if (ch.response_errors && Object.keys(ch.response_errors).length) {
        return { ok: false, reason: LoginFailureReason.INVALID_CREDENTIALS };
      }
      if (ch.component === "ak-stage-identification") {
        ...
        return { ok: false, reason: LoginFailureReason.UNSUPPORTED_STAGE };
      }
    }
    if (
    ...
    if (!code) return { ok: false, reason: LoginFailureReason.ERROR };
    ...
    if (!tokenRes.ok) return { ok: false, reason: LoginFailureReason.ERROR };
