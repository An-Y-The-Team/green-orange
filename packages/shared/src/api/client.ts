...
class ApiClient {
  ...
  /**
   * Update the client configuration after construction.
   * Useful for setting baseURL from app-specific env config.
   */
...
// Export a default instance with session cookie credentials.
// Apps should call `api.configure({ baseURL })` at startup to set the backend URL.
export const api = new ApiClient({ credentials: "include" });
