...
There are **two** CRM backends and, as of the 2026-08 sync, they implement the **same
v2 contract** — same endpoints, same payloads, same rules — so `crm-web` works
pointed at either via `CRM_API_URL`. `apps/crm-api-nest` (NestJS + Prisma + Bun,
port 8001) stays the **production default**; `apps/crm-api` (Python + FastAPI +
SQLModel, port 8000) is the teaching/sandbox implementation of that same contract.

**Change one, change the other.** A behaviour that exists in only one backend is a
